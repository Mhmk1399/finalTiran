...
const ImageUploadModal: React.FC<ImageUploadModalProps> = ({
  ...
  const uploadImage = useCallback(async (file: File): Promise<string> => {
    const token = localStorage.getItem("token"); // Get token from localStorage (assuming it's stored there)
    if (!token) {
  ...
  const handleThumbnailUpload = async (
    ...
    if (thumbnails.length + files.length > 4) {
      // Since total images <=5, main +4 thumbnails
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-6">
              {mainImage ? (
                ...
                <label className="cursor-pointer block text-center">
                  ...
                  <p className="text-sm text-gray-500">JPG, PNG, GIF تا 30MB</p>
                  <input
                    type="file"
                    accept="image/*"
